...
static SemaphoreHandle_t xBusSemaphore;
...
static void m5gfx_lvgl_flush(lv_disp_drv_t *disp, const lv_area_t *area,
    ...
    // Lock bus while writing to display to avoid conflicts with SD SPI
    if (xBusSemaphore) {
        xSemaphoreTake(xBusSemaphore, portMAX_DELAY);
    }
...
void m5gfx_lvgl_init(void) {
    ...
    xBusSemaphore                                     = xSemaphoreCreateMutex();
